...
const API_BASE_URL = "https://art-ecologique-backend-production.up.railway.app";
...
function SellerApp({ products, setProducts, orders, storeId }) {
  ...
  const pendingPayout = myOrders.filter((o) => o.payout === "pending").reduce((s, o) => s + o.net, 0);
  ...
  const addProduct = () => {
    ...
    // In the real app this uploads each photo to POST /seller/products/:id/images
    // (see backend/src/routes/seller.js) and stores the returned Cloudinary URLs.
    // Here in the preview, we use the local picked-photo previews directly.
    const cover = newPhotos[0]?.previewUrl || `https://picsum.photos/seed/${encodeURIComponent(newProduct.name)}/600/600`;
    setProducts((ps) => [...ps, {
      id: "p" + (ps.length + 1), storeId, name: newProduct.name, category: newProduct.category,
      price: Number(newProduct.price), stock: Number(newProduct.stock) || 1, sold: 0, rating: 5,
      img: cover, gallery: newPhotos.map((p) => p.previewUrl),
      desc: "Newly listed piece.",
    }]);
  ...
  return (
    <div>
      ...
      {section === "overview" && (
        <div>
          <SectionHeader eyebrow="Lagos Loom Studio" title="Store overview" sub="Last 30 days performance across your storefront." />
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <StatCard label="Gross sales" value={fmtN(totalSales)} icon={DollarSign} trend={12} />
            <StatCard label="Net earnings" value={fmtN(netEarnings)} icon={Wallet} accent={COLOR.gold} sub="after 5% commission" />
            <StatCard label="Orders" value={myOrders.length} icon={ShoppingCart} accent={COLOR.sage} trend={8} />
            <StatCard label="Pending orders" value={pending} icon={Clock} accent={COLOR.rust} />
          </div>
      ...
      {section === "payouts" && (
        <div>
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-6">
            ...
            <StatCard label="Total paid out" value={fmtN(netEarnings - pendingPayout)} icon={CheckCircle2} accent={COLOR.forest} />
          ...
          <div className="rounded-2xl p-5 flex items-center justify-between flex-wrap gap-3" style={{ background: "#fff", border: `1px solid ${COLOR.line}` }}>
            ...
            <button className="px-5 py-2.5 rounded-full text-sm font-semibold" style={{ background: COLOR.gold, color: COLOR.forestDeep }}>Withdraw {fmtN(pendingPayout)}</button>
          </div>
          <div className="mt-6 space-y-2">
            {myOrders.filter((o) => o.payout === "paid").slice(0, 6).map((o) => (
              <div key={o.id} className="flex items-center justify-between text-sm rounded-xl p-3" style={{ background: "#fff", border: `1px solid ${COLOR.line}` }}>
                <span style={{ color: COLOR.bark }}>{o.id} · {o.date}</span>
                <span className="font-medium" style={{ color: COLOR.ink }}>{fmtN(o.net)}</span>
...
function OrdersTable({ orders, showBuyer, showStore, editableStatus }) {
  ...
  const cycle = { pending: "processing", processing: "shipped", shipped: "delivered", delivered: "delivered" };
  return (
      ...
      <table className="w-full text-sm">
        <thead>
          <tr style={{ background: COLOR.creamDeep }}>
            <th className="text-left px-4 py-3 font-medium" style={{ color: COLOR.bark }}>Order</th>
            <th className="text-left px-4 py-3 font-medium" style={{ color: COLOR.bark }}>Item</th>
            {showBuyer && <th className="text-left px-4 py-3 font-medium" style={{ color: COLOR.bark }}>Buyer</th>}
            <th className="text-left px-4 py-3 font-medium" style={{ color: COLOR.bark }}>Gross</th>
        ...
        <tbody>
          {orders.slice(0, 12).map((o) => {
            ...
            return (
              <tr key={o.id} className="border-t" style={{ borderColor: COLOR.line }}>
                <td className="px-4 py-3" style={{ color: COLOR.ink }}>{o.id}</td>
                <td className="px-4 py-3" style={{ color: COLOR.bark }}>{o.productName} × {o.qty}</td>
                {showBuyer && <td className="px-4 py-3" style={{ color: COLOR.bark }}>{o.buyer}</td>}
                <td className="px-4 py-3 font-medium" style={{ color: COLOR.forest }}>{fmtN(o.gross)}</td>
                <td className="px-4 py-3"><StatusTag status={status} /></td>
                {editableStatus && (
                  <td className="px-4 py-3">
                    {status !== "delivered" && (
                      <button onClick={() => setLocalStatus((s) => ({ ...s, [o.id]: cycle[status] }))} className="text-xs font-semibold flex items-center gap-1" style={{ color: COLOR.forest }}>
...
function AdminApp({ products, orders, users, setUsers, stores, setStores }) {
  ...
  const toggleStore = (id) => setStores((ss) => ss.map((s) => s.id === id ? { ...s, status: s.status === "active" ? "suspended" : "active" } : s));
  const approveStore = (id) => setStores((ss) => ss.map((s) => s.id === id ? { ...s, status: "active" } : s));
    ...
    <div>
      ...
      {section === "financial" && (
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <StatCard label="Gross marketplace sales" value={fmtN(grossRevenue)} icon={DollarSign} trend={9} />
            <StatCard label="Platform commission (5%)" value={fmtN(commission)} icon={TrendingUp} accent={COLOR.gold} />
            <StatCard label="Seller payouts" value={fmtN(payouts)} icon={Wallet} accent={COLOR.sage} />
            <StatCard label="Pending payouts" value={fmtN(pendingPayouts)} icon={Clock} accent={COLOR.rust} />
          </div>
...
export default function ArtEcologique() {
  ...
  const [orders] = useState(seedOrders(seedProducts()));
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/products`)
      ...
      .then((data) => {
        ...
        const demoByName = Object.fromEntries(seedProducts().map((p) => [p.name, p]));
        const apiProducts = data.map((p) => {
          const demo = demoByName[p.name] || {};
          const store = STORES.find((s) => s.name === p.store?.name);
          return {
            ...demo,
            id: p.id,
            storeId: store?.id || p.storeId,
            name: p.name,
            category: p.category,
            price: Math.round((p.priceKobo || 0) / 100),
            stock: p.stock,
            img: p.images?.[0] || demo.img || "https://picsum.photos/seed/art-ecologique/600/600",
      ...
      <div className="sticky top-0 z-40 backdrop-blur" style={{ background: COLOR.cream + "e6", borderBottom: `1px solid ${COLOR.line}` }}>
        <div className="max-w-6xl mx-auto px-5 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <div className="flex gap-1.5 rounded-full p-1" style={{ background: COLOR.creamDeep }}>
              {["consumer", "seller", "admin"].map((r) => (
                <button
                  key={r} onClick={() => setRole(r)}
                  className="px-4 py-1.5 rounded-full text-xs font-semibold capitalize transition-all"
                  style={{ background: role === r ? COLOR.forest : "transparent", color: role === r ? COLOR.cream : COLOR.bark }}
                >
                  {r}
                </button>
              ))}
  ...
  />
        ...
        {role === "seller" && <SellerApp products={products} setProducts={setProducts} orders={orders} storeId="s1" />}
        {role === "admin" && <AdminApp products={products} orders={orders} users={users} setUsers={setUsers} stores={stores} setStores={setStores} />}
      </div>
